...
function SideProductsNavbar() {
    ...
    const getTypeUnicsData = (data, type) => {
        let newCaregories = data?.map((cureElem) => cureElem[type].trim().toLowerCase())
        return newCaregories = ["All", ...new Set(newCaregories)]
    }
    const productTypeOnlyData = getTypeUnicsData(all_products, "productType")
    console.log("productTypeOnlyData",productTypeOnlyData);
    

    const getunicsData = (data, type) => {
        let newPtype = data?.filter((producttype) => producttype.productType.toLowerCase() == targetVlaue)
            ...
            <div className="px-3 py-5 category-font">
                ...
                {maincategory.map(({ title, items }) => (
                    <div key={title} className="pl-2">
                        <h3 className="text-lg" onClick={() => toggleCategory(title)}>{title}</h3>
                        {toggle[title] && items ? (
                            <div>
                                {items ? (
                                    Object.entries(items).map(([subCategory, subItems]) => (
                                        <div key={subCategory} className="pl-2">
                                            <h5 className="text-[16px] cursor-pointer hover:text-[#3d6aff]" onClick={() => SubHandleToggle(title, subCategory)}>{subCategory.charAt(0).toUpperCase() + subCategory.slice(1).toLowerCase()}</h5>
                                            {subToggle[`${title}_${subCategory}`] && subItems ? (
                                                <ul className="pl-2">
                                                    {Array.isArray(subItems) && subItems.length > 0 ? (subItems.map((item, index) => (
                ...
                <div className="pt-5">
                    ...
                    <input type="range" min={248} max={1000000} value={rangSelecter} onChange={(e) => setRangSelecter(e.target.value)} />
                </div>
                ...
                <div className="py-5">
                    ...
                    <div className="py-3">
                        <select id="countries" onClick={updateFilter} onChange={(e)=>setTargetVlaue(e.target.value)} name='productType' className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            {productTypeOnlyData.map((UnicsCategories) => {
                                return <option value={UnicsCategories} >{UnicsCategories}</option>
                            })}
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    {companyOnlyData.map((comapny, i) => {
                        return <>
                            <div key={i} className="flex gap-3 items-center pl-2 ">
                                <input  onClick={updateFilter} type="radio" value={comapny} name="company" className=" focus:ring-2 ring-slate-600 rounded-full focus:ring-offset-2 " />
                                <div>{comapny}</div>
                            </div>
                        </>
                    })}
