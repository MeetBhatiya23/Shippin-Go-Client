...
function SingleProduct() {
  ...
  const { _id } = useParams();
  const { getSingleProduct, isSingleLoding, singleProducts } =
    useProductContext();
  ...
  const [colourCollection, setcolourCollection] = useState(
    colours ? colours[0] : null
  );

  useEffect(() => {
    // console.log("hello guys i am check out");
    window.scrollTo(0, 0);
  }, []);
  ...
  useEffect(() => {
    ...
    if (!Object.keys(singleProducts)?.length) {
      ...
      Object.keys(singleProducts)?.length &&
      singleProducts?._id != _id
    ) {
      getSingleProduct(`${API}/${_id}`);
    }
  }, [_id]);
    ...
    <>
                  ...
                  {selectedColorq.slice(0, 6).map((imgSrc, index) => {
                    return (
                      <>
                        <div key={index}>
                          <button
                            className=" border border-[#0f0f0f] rounded-[10px]"
                            onClick={() => setSelectedHoverImg(imgSrc)}
                          >
                            <img
                              src={imgSrc}
          ...
          <div className="flex flex-col gap-7 ">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex justify-start items-end gap-2">
                <div className="text-lg text-gray-700 ">Deal of the Day :</div>{" "}
                <div className="text-4xl ">
                  <FormetPrice price={(price * (100 - offers)) / 100} />
                </div>
                {offers > 0 && (
                        ...
                        return (
                          <>
                            <button
                              key={index}
                              onClick={() => {
                                setSelectedColorq(images[Pcol]);
                                setcolourCollection(Pcol);
                              }}
            ...
            <div>
              ...
              <div className="flex gap-4">
                {size?.map((data) => {
                  return (
                    <div class="flex items-center mb-4 border border-collapse py-2 px-3 rounded-xl ">
                      <input
                        id="default-radio-1"
                        type="radio"
                        value={data}
      ...
      {toggle && (
        <div
          ...
          tabIndex="-1"
          className="px-40 bg-[#acabab50]  h-screen w-screen fixed top-0 right-0 left-0 justify-center items-center md:inset-0 z-[1000]"
        >
              ...
              <div className=" space-y-4 bg-white flex items-center justify-end gap-[300px] w-full ">
                <div className="relative left-12">
                  {selectedhoverImg?.length ? (
                    <>
                      <img
                        className="w-[500px] h-[500px] object-scale-down relative left-[125px]"
                        src={selectedhoverImg}
                        alt={modelName}
                      />
                    </>
                  ) : (
                    ...
                    {selectedColorq.map((toggleImg, index) => {
                      return (
                        <>
                          <button
                            key={index}
                            onClick={() => setSelectedHoverImg(toggleImg)}
                            className="border border-[2px] m-1 rounded-[5px] border-[#0f0f0f]"
                          >
