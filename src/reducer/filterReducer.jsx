function filterReducer(state, action) {
    switch (action.type) {
        ...
        case "UPDATE_FILTER_DATA":
            ...
            console.log("Update the filter state (e.g., text input)", { [name]: value });
        ...
        case "FILTER_PRODUCT":
            ...
            if (text && text.trim() !== "") {
                ...
                tempFilterProduct = tempFilterProduct.filter((product) =>
                    product.title && product.title.toLowerCase().includes(text.toLowerCase())
                );
            }
            ...
            if (productType) {
                if (productType === "All") {
                    ...
                    tempFilterProduct = tempFilterProduct.filter((curElem) => {
                        return curElem.productType.trim().toLowerCase() === productType?.trim().toLowerCase()
                    })
                }
            }
            ...
            if (company) {
                if (company === "All") {
                    tempFilterProduct = tempFilterProduct.filter((data)=>{
                        return data.productType.trim().toLowerCase() === productType?.trim().toLowerCase()
                    })
                }
                else {
                    console.log("tempFilterProduct", company);
                    tempFilterProduct = tempFilterProduct.filter((curElem) => {
                        return curElem.company.trim().toLowerCase() === company?.trim().toLowerCase()
                    })
                }
            }
            return {
